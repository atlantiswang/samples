...
BOOL CAdvComboBox::Create( DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID )
{
	...
	return CWnd::Create(NULL, _T(""), dwStyle, m_rcCombo, pParentWnd, nID );
}
...
void CAdvComboBox::OnPaint() 
{
	...
	//	clrOldTextColor = dc.SetTextColor( ::GetSysColor(COLOR_HIGHLIGHTTEXT) );
		...
		if( m_bHasFocus && !m_bDropListVisible )
		{
			...
			dc.DrawText( m_strEdit, &rcText, DT_SINGLELINE|DT_VCENTER);
		}
		else
		{
...
int CAdvComboBox::GetLBText(int nIndex, LPTSTR lpszText)
{
	...
	_tcscpy_s( lpszText, m_iter->strText.GetLength(), m_iter->strText);
	return m_iter->strText.GetLength()+1;
}
...
int CAdvComboBox::GetText(LPTSTR lpszText)
{
	...
	{	
		...
		_tcscpy_s( lpszText, str.GetLength(),str );
		return str.GetLength();
	}
	else
	{
		_tcscpy_s( lpszText, m_strEdit.GetLength(),m_strEdit);
...
int CAdvComboBox::FindString(int nStartAfter, LPCTSTR lpszString)
{
	...
	{
		if( _tcsncmp( m_iter->strText.GetBuffer(0), lpszString, _tcslen(lpszString) ) == 0 )
		{
			return nPos;
		}
		nPos++;
...
int CAdvComboBox::FindStringExact(int nIndexStart, LPCTSTR lpszFind)
{
	...
	for( m_iter; m_iter != m_list.end(); ++m_iter )
	{
		if(  m_iter->strText.Compare(lpszFind) == 0 )
		{
			return nPos;
		}
		nPos++;
...
int CAdvComboBox::SelectString(int nStartAfter, LPCTSTR lpszString)
{
	...
	for( m_iter; m_iter != m_list.end(); ++m_iter )
	{
		if( m_iter->strText.Compare(lpszString)  == 0 )
		{
			m_nCurSel = nPos;
			m_strEdit = m_iter->strText;
			Invalidate();
		...
		if( !bAutoAppendInProgress )
		{
			if( m_dwACBStyle & ACBS_AUTOSUGGEST )
			{
				...
				while( m_iter != m_list.end() )
				{
					...
					//////////////////////////////////////////////////////////////////////////
					CString strText(m_iter->strText);
					CString strPy = CDataProcess::GetFirstLetter(strText);
					strPy.MakeUpper();
					CString strFind(str);
					strFind.MakeUpper();
					int nPos = strPy.Find(strFind,0);
					//////////////////////////////////////////////////////////////////////////
				//	int nPos = m_iter->strText.find( str, 0 );
					if( nPos == 0 )
					{
