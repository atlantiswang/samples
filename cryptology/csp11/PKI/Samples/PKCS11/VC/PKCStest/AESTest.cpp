...
AESTest::AESTest(char* dll_file_path):CP11TestBase(dll_file_path)
{
	m_hKey = 0;
}
...
void AESTest::GenerateKey()
{
	...
	CK_ULONG ulCount = 7;
	CK_RV rv = m_p11->C_GenerateKey(m_hSession, &mechanism, Aestem, ulCount, &m_hKey); 
...
void AESTest::crypt_Update()
{
	...
	for(int i=0;i<3;i++)
	{
		...
		rv = m_p11->C_EncryptUpdate(m_hSession, bIn, ulEnc1stPice, NULL, &ulTemp);//get buffer's size.
		if(CKR_OK == rv)
		...
		rv = m_p11->C_EncryptUpdate(m_hSession,  &(bIn[ulEnc1stPice]), ulIn-ulEnc1stPice, NULL, &ulTemp);//get buffer's size.
		if(CKR_OK == rv)
		...
		rv = m_p11->C_EncryptFinal(m_hSession, NULL, &ulTemp);//Get buffer's size:
		if(CKR_OK == rv)
		...
		rv = m_p11->C_DecryptUpdate(m_hSession, bTemp, ulDec1stPice, NULL, &ulOut);//Get buffer's size
		if(CKR_OK == rv)
		...
		//Get decrypted data:
		rv = m_p11->C_DecryptUpdate(m_hSession, &(bTemp[ulDec1stPice]), ulEncrypted-ulDec1stPice, NULL, &ulOut);//Get buffer's size
		if(CKR_OK == rv)
		...
		rv = m_p11->C_DecryptFinal(m_hSession, NULL, &ulOut);//Get buffer's size
		if(CKR_OK == rv)
